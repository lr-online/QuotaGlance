#!/usr/bin/env bash
# Sync the full contract tree (Providers + Aggregation + Alerts + RefreshLifecycle + ServiceStatus) into the
...
mkdir -p "$TARGET_DIR/aggregation" "$TARGET_DIR/alerts" "$TARGET_DIR/refreshlifecycle" "$TARGET_DIR/servicestatus"
...
cp -R "$SOURCE_DIR/ServiceStatus/." "$TARGET_DIR/servicestatus/"

echo "Synced provider, aggregation, alert, refresh-lifecycle, and service-status contracts to Windows test resources."
