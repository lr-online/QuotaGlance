  ...
  return (
    ...
    >
      ...
      {usage ? <>
        ...
        {usage.quota_windows.length ? <Card title={t("metrics.quotaWindow")} className="p-4"><dl className="divide-y divide-black/5 dark:divide-white/10">{usage.quota_windows.map((window, index) => <div key={`${window.label}-${index}`} className="flex items-center justify-between gap-3 py-3"><dt><p className="text-sm font-medium">{window.label}</p><p className="mt-1 text-xs text-qg-neutral">{window.unit}{window.resets_at ? ` · ${new Date(window.resets_at).toLocaleString()}` : ""}</p></dt><dd className="font-mono text-sm tabular-nums">{window.used ?? "--"} / {window.limit ?? "--"}</dd></div>)}</dl></Card> : null}
        {usage.daily_usage.length ? <Card title={t("detail.dailyUsage")} className="p-4"><dl className="divide-y divide-black/5 dark:divide-white/10">{usage.daily_usage.slice(-7).reverse().map((day) => <div key={day.date} className="flex justify-between gap-3 py-3 text-sm"><dt>{day.date}</dt><dd className="font-mono tabular-nums">{formatMoney(day.actual_cost)}{day.requests !== undefined ? ` · ${formatNumber(day.requests)}` : ""}</dd></div>)}</dl></Card> : null}
        {usage.model_usage.length ? <Card title={t("detail.modelUsage")} className="p-4"><dl className="divide-y divide-black/5 dark:divide-white/10">{usage.model_usage.map((model, index) => <div key={`${model.model}-${index}`} className="flex justify-between gap-3 py-3 text-sm"><dt className="min-w-0 truncate">{model.model}</dt><dd className="shrink-0 font-mono tabular-nums">{formatMoney(model.actual_cost)}{model.requests !== undefined ? ` · ${formatNumber(model.requests)}` : ""}</dd></div>)}</dl></Card> : null}
